...
describe("OpenMeteoWeatherProvider", () => {
  ...
  it("does not request weather outside the 16-day forecast range", async () => {
    ...
    await expect(new OpenMeteoWeatherProvider().forecast({ latitude: 37.57, longitude: 126.98, startDate: "2026-10-10", tripDays: 2, today: "2026-09-15" }))
  ...
  it("normalizes daily weather", async () => {
    ...
    const result = await new OpenMeteoWeatherProvider().forecast({ latitude: 37.57, longitude: 126.98, startDate: "2026-09-16", tripDays: 1, today: "2026-09-15" });
