...
import { Button, Card as AnimalCard } from "animal-island-ui";
...
function Preview({ kind }: { kind: (typeof steps)[number]["kind"] }) {
  ...
  if (kind === "plan") return <div className={styles.panel}><StepCard number="1" title="说出大概想法" text="和朋友去韩国，想吃好吃的。" color="app-green" /><StepCard number="2" title="逐个补齐关键信息" text="一次只问一个最重要的问题。" color="app-blue" /><StepCard number="3" title="得到可以执行的推荐" text="真实地点、逐日安排、费用参考。" color="app-yellow" /></div>;
...
function StepCard({ number, title, text, color }: { number: string; title: string; text: string; color: "app-green" | "app-blue" | "app-yellow" }) {
  return <AnimalCard color={color} pattern={color} className={styles.card}><b>{number}</b><div><strong>{title}</strong><small>{text}</small></div></AnimalCard>;
}
